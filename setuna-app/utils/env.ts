...
const getLocalIPAddress = (): string => {
  ...
  // For now, return a list of common IPs to try
  if (Platform.OS === 'ios') {
    // iOS simulator typically uses localhost
    return 'localhost';
  } else if (Platform.OS === 'android') {
    // Android emulator - try 10.0.2.2 first, then localhost
    return '10.0.2.2';
  }
...
// Function to get network IP address (for physical devices)
const getNetworkIPAddress = (): string => {
  // Common network IP ranges - will be tested automatically
  return 'localhost';
};
...
export const getIPsToTry = (): string[] => {
  ...
  // List of IPs to try in order of likelihood
  const ipsToTry = [
    '10.77.207.119', // User's fixed device IP (Prioritized)
    platformIP, // Platform-specific IP
...
export const ENV_CONFIG = {
  ...
  development: {
    ...
    NETWORK_IP: getNetworkIPAddress(),

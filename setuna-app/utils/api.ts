...
import { getWorkingBackendURL } from './connectivity';
...
const setupInterceptors = () => {
  // Add request interceptor for authentication and dynamic URL
  API.interceptors.request.use(
    async (config) => {
      try {
        // Get the current working backend URL - this will scan if needed
        const workingURL = await getWorkingBackendURL();

        // Update base URL if it changed
        if (workingURL && workingURL !== config.baseURL) {
        ...
        // Fallback to default, but the scanner should have handled it
        if (!config.baseURL) {
    ...
    async (error) => {
      ...
      if (error.code === 'NETWORK_ERROR' || error.message.includes('Network Error')) {
        ...
        if (!originalRequest._retry) {
          ...
          // Try to get a fresh working URL
          const newURL = await getWorkingBackendURL();
          originalRequest.baseURL = newURL;
          console.log('🔄 Retrying with discovered URL:', originalRequest.baseURL);
      ...
      // Handle network errors with automatic retry and URL switching
      if (error.request && !error.response) {
        console.log('🔄 Network error detected, attempting to find working backend...');

        try {
          // Test connectivity before retrying
          const { testConnectivity } = await import('./connectivity');
          const isBackendAccessible = await testConnectivity();

          if (isBackendAccessible) {
            console.log('🔄 Backend is accessible, retrying request...');
            const newWorkingURL = getWorkingBackendURL();
            originalRequest.baseURL = newWorkingURL;
            return API(originalRequest);
          } else {
            console.log('❌ Backend is not accessible');
            console.log('💡 Make sure your Django backend is running: python manage.py runserver 0.0.0.0:8000');
            console.log('💡 If this persists, run: node scripts/update-local-ip.js');
          }
        } catch (refreshError) {
          console.log('⚠️ Failed to test connectivity:', refreshError);
        }
      }

      // Handle common errors
      if (error.response) {
        // Use console.warn for API errors to prevent user-facing error displays
        console.warn('API Response Error:', error.response.status, error.response.data);
      } else if (error.request) {
        console.warn('Network Error: Backend might not be running or accessible. Request:', error.request);
      } else {

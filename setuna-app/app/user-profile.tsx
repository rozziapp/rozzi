...
export default function UserProfileScreen() {
  ...
  const fetchUserPosts = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data && response.data.results) {
        console.log('All posts from API:', response.data.results);
        ...
        const posts = response.data.results.filter((post: any) => {
          console.log('Post user ID:', post.user?.id, 'Profile user ID:', userIdString);
          return post.status === 'Open' &&
            (post.post_type === 'hire' || post.post_type === 'looking') &&
            post.user?.id == userIdString; // Ensure it's the same user
        });

        console.log('Filtered posts for profile:', posts);
        setUserPosts(posts);
      }
    } catch (error: any) {
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      if (conversation) {
        ...
        router.push(`/chat?conversationId=${conversation.id}&userId=${userProfile.id}&userName=${userProfile.full_name || userProfile.username}&userImage=${userProfile.profile_picture || ''}`);
      } else {
  ...
  const handleShare = () => {
    if (userProfile) {
      const shareText = `Check out ${userProfile.full_name}'s profile on Setuna!`;

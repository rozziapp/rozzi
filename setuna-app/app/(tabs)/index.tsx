...
export default function HomeScreen() {
  ...
  // Fetch notification count from local storage
  const fetchNotificationCount = async () => {
    try {
      ...
      if (localData) {
        ...
        const userNotifications = localNotifications.filter((notification: any) =>
          notification.recipient?.id === user?.id && !notification.read
        );
  ...
  // Refresh data when screen comes into focus (e.g., returning from posting a job)
  useFocusEffect(
    React.useCallback(() => {
      if (isAuthenticated && !isLoading) {
        fetchJobs(); // Refresh jobs to show new posts
        fetchFollowedUsers();
        fetchNotificationCount();
      }
    }, [isAuthenticated, isLoading])
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      // Fetch applied jobs to check which ones user has already applied to
      try {
      ...
      // Recalculate user engagement after fetching new posts
  ...
  const handleSharePost = (post: JobPost) => {
    try {
      ...
      const shareText = `Check out this job opportunity: ${post.title}\n\nDescription: ${post.description}\n\nLocation: ${post.location}\nSalary: ${formatSalary(post.salary_min, post.salary_max)}\nJob Type: ${post.job_type}\n\nPosted by: ${post.user?.full_name || 'Unknown'}`;
        ...
        [
          {
            ...
            onPress: () => {
              // In a real app, use Clipboard.setString(shareText)
              console.log('Would copy to clipboard:', shareText);
              Alert.alert('Copied!', 'Job details copied to clipboard');
              setShowOptionsMenu(null);
            }
          },
          {
    ...
    return (
      ...
      <View style={[styles.topBar, { backgroundColor: '#B0AAD9' }]}>
          <View style={styles.leftContainer}>
            <Text style={[styles.appName, { color: '#fff', fontFamily: 'IrishGrover' }]}>Setuna</Text>
          </View>
          <View style={styles.centerContainer}>
            <View style={[styles.searchContainer, { backgroundColor: 'rgba(255, 255, 255, 0.2)', width: 120, height: 40, paddingHorizontal: 25, borderRadius: 100 }]}>
        ...
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tagsContainer} contentContainerStyle={styles.tagsContent}>
            {[1, 2, 3, 4, 5].map((i) => (
              <View key={i} style={[styles.tag, { backgroundColor: 'rgba(255, 255, 255, 0.3)', borderColor: 'rgba(255, 255, 255, 0.2)' }]}>
                <View style={{ width: 40, height: 16, backgroundColor: 'rgba(255, 255, 255, 0.4)', borderRadius: 8 }} />
              </View>
            ))}
          </ScrollView>
          ...
          <View style={styles.postsContainer}>
            {[1, 2, 3, 4].map((i) => (
              <View key={i} style={[styles.postCard, { backgroundColor: 'rgba(255, 255, 255, 0.9)' }]}>
                {/* Skeleton Header */}
        ...
        <View style={styles.bottomNavContainer}>
          <View style={styles.bottomNavCapsule}>
            ...
            <View style={[styles.postButton, { backgroundColor: 'rgba(107, 70, 193, 0.7)' }]} />
  ...
  if (error) {
    return (
      <View style={[styles.container, { backgroundColor: 'rgb(167, 139, 250)', justifyContent: 'center', alignItems: 'center' }]}>
    ...
    <View style={[styles.container, { backgroundColor: '#B0AAD9' }]}>
      ...
      <View style={[styles.topBar, { backgroundColor: '#B0AAD9' }]}>
        ...
        >
          ...
          >
            <Animated.View style={{ transform: [{ scale: isSearchFocused ? 1.1 : 1 }] }}>
              <Ionicons
                name="search"
        ...
        <View style={styles.rightContainer}>
          <TouchableOpacity
            ...
            onPress={() => router.push('/find-users')}
          >
        ...
        >
          {jobTags.map((tag) => (
            <TouchableOpacity
              ...
              style={[
                styles.tag,
                selectedTag === tag && { backgroundColor: '#6b46c1' }
              ]}
            ...
            refreshing ? (
              ...
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  {searchQuery.trim() ? 'No exact matches found' : 'No jobs available'}
                </Text>
                <Text style={styles.emptyStateSubtext}>
                  {searchQuery.trim()
                    ? `Try searching for: "${searchQuery.trim().replace(/[-_.,!@#$%^&*()+=]/g, ' ').replace(/\s+/g, ' ').trim()}" in different ways or check similar terms`
              ...
              return (
                ...
                >
                  ...
                  <View style={[
                    ...
                    !isLookingPost && styles.hireDescriptionContainer
                  ]}>
                    <Text style={[
                      styles.description,
                  ...
                  {expandedPost === post.id && (
                    <View style={styles.expandedContent}>
                      ...
                      <View style={styles.expandedDetailsSection}>
                        <Text style={styles.expandedSectionTitle}>Additional Details</Text>
                        <View style={styles.detailsGrid}>
                          <View style={styles.detailItem}>
                            <Text style={styles.detailLabel}>Sector</Text>
                            <Text style={styles.detailValue}>{post.sector || 'Professional'}</Text>
                          </View>
                          <View style={styles.detailItem}>
                            <Text style={styles.detailLabel}>Shift Timing</Text>
                            <Text style={styles.detailValue}>{post.shift_timing || 'not mentioned'}</Text>
                          </View>
                          <View style={styles.detailItem}>
                          ...
                          {!isLookingPost && (
                            <View style={styles.detailItem}>
                              ...
                              <Text style={styles.detailValue}>{post.applicants_count}</Text>
                            </View>
                          )}
                          {post.is_remote && (
                            <View style={styles.detailItem}>
                              <Text style={styles.detailLabel}>Remote Work</Text>
                              <Text style={styles.detailValue}>Available</Text>
                            </View>
                          )}
                          {(post.address || post.pincode) && (
                            <View style={styles.detailItem}>
                              <Text style={styles.detailLabel}>Complete Address</Text>

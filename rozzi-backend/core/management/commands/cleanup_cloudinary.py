    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not old_files_only:
                self.stdout.write('[SEARCH] Scanning for orphaned files...')
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'[OK] Orphaned files cleanup: {deleted_count} deleted, {error_count} errors'
                    )
                )
            ...
            if not orphaned_only:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'[OK] Old files cleanup: {deleted_count} deleted, {error_count} errors'
                    )
                )
            ...
            if total_errors == 0:
                ...
                self.stdout.write(
                    self.style.WARNING(f'\n[WARN]️  Cleanup completed with {total_errors} errors')
                )

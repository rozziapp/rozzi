...
interface Applicant {
  ...
  resume_url?: string; // Direct Google Drive URL submitted with application
}
...
export default function JobDetailsScreen() {
  ...
  const getOptimizedCloudinaryUrl = (originalUrl: string, fileName: string) => {
    // For Cloudinary raw files, use the original URL as-is
    // Adding extensions or modifying the URL can cause 401/403 errors
    if (!originalUrl.includes('cloudinary.com')) {
      return originalUrl;
    }

    // For raw uploads, return the URL as-is - don't modify it
    // Cloudinary raw resources are accessed by their exact public_id
    if (originalUrl.includes('/raw/upload/')) {
      console.log('Using original Cloudinary raw URL (no modification):', originalUrl);
      return originalUrl;
    }

    // For image/video resources, standard transformations can be applied
    console.log('Cloudinary URL (non-raw):', originalUrl);
    return originalUrl;
  };
  ...
  const handleDownloadResume = async (resumeUrl: string, fileName: string) => {
    try {
      ...
      // For Cloudinary URLs, try to get a signed URL for secure access
      let downloadUrl = resumeUrl;
      if (resumeUrl.includes('cloudinary.com')) {
        console.log('Processing Cloudinary URL for secure download');

        try {
          // Request signed URL from backend
          const response = await API.post('/resume-files/signed-url/', {
            file_url: resumeUrl
          });

          if (response.data && response.data.signed_url) {
            console.log('Received signed URL from backend');
            downloadUrl = response.data.signed_url;
          } else {
            console.log('No signed URL returned, using optimized URL');
            downloadUrl = getOptimizedCloudinaryUrl(resumeUrl, fileName);
          }
        } catch (error) {
          console.error('Failed to get signed URL:', error);
          console.log('Falling back to optimized standard URL');
          downloadUrl = getOptimizedCloudinaryUrl(resumeUrl, fileName);
        }
      ...
      if (downloadResult.status !== 200) {
        ...
        if (resumeUrl.includes('cloudinary.com')) {
          ...
          } else if (downloadResult.status === 403 || downloadResult.status === 401) {
            // Access control issue - offer to open in browser instead
            console.log('Cloudinary access denied, offering browser fallback');
            Alert.alert(
              'Download Restricted',
              'This file has access restrictions. Would you like to open it in your browser instead?',
              [
  ...
  const handleDownloadToDevice = async (resumeUrl: string, fileName: string) => {
    try {
      ...
      if (resumeUrl.includes('cloudinary.com')) {
        console.log('Processing Cloudinary URL for download with signed URL');
        ...
        try {
          ...
          if (response.data && response.data.signed_url) {
            console.log('Received signed URL from backend for browser download');
            targetUrl = response.data.signed_url;
          } else {
            console.log('No signed URL returned, using standard flow');
            const optimizedUrl = getOptimizedCloudinaryUrl(resumeUrl, fileName);
            targetUrl = buildCloudinaryAttachmentUrl(optimizedUrl, fileName);
          }
        } catch (error) {
          console.error('Failed to get signed URL:', error);
          const optimizedUrl = getOptimizedCloudinaryUrl(resumeUrl, fileName);
        ...
        console.log('Final browser target URL:', targetUrl);
  ...
  const handleDownloadTable = async () => {
    try {
      const data = applicants.map(app => ({
        Name: app.user.full_name || app.user.username,
        Age: app.id_card?.age ? app.id_card.age.toString() : calculateAge(app.id_card?.date_of_birth),
        Gender: app.id_card?.gender || 'N/A',
        Phone: app.id_card?.phone_number || 'N/A',
        Address: app.id_card?.address || 'N/A',
        Skills: app.id_card?.skills?.join(', ') || 'N/A',
        Resume: app.resume_url || app.resume?.file_url || 'No Resume'
      }));

      const jsonString = JSON.stringify(data, null, 2);
      const fileUri = FileSystem.cacheDirectory + 'applicants_data.json';
      ...
      await Sharing.shareAsync(fileUri, {
        ...
        dialogTitle: 'Download Applicants Data',
        UTI: 'public.json'
      });
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.jobCard}>
          ...
          <View style={styles.jobInfoGrid}>
            <View style={styles.jobInfoItem}>
              ...
              <Text style={styles.jobInfoText}>{formatSalary(jobDetails.salary_min, jobDetails.salary_max, jobDetails.job_type)}</Text>
            </View>
            <View style={styles.jobInfoItem}>
          ...
          <View style={styles.modalContent}>
            ...
            {selectedApplicant && (
              <ScrollView style={styles.modalScrollView} showsVerticalScrollIndicator={false}>
                ...
                {/* Resume */}
                {selectedApplicant.resume && selectedApplicant.resume.file_url && (
                  <View style={styles.modalSection}>
                    <Text style={styles.modalSectionTitle}>Resume</Text>
                    <TouchableOpacity
                      style={styles.resumeUrlContainer}
                      onPress={() => Linking.openURL(selectedApplicant.resume!.file_url)}
                    >
                      <View style={styles.resumeUrlIcon}>
                        <Ionicons name="document-text" size={22} color="#8b5cf6" />
                      </View>
                      <View style={styles.resumeUrlInfo}>
                        <Text style={styles.resumeUrlLabel}>{selectedApplicant.resume.file_name}</Text>
                        <Text style={styles.resumeUrlText} numberOfLines={1}>
                ...
                {selectedApplicant.resume_url && (
                  <View style={styles.modalSection}>
                    <Text style={styles.modalSectionTitle}>Resume Link</Text>
                    <TouchableOpacity
                      style={styles.resumeUrlContainer}
                      onPress={() => Linking.openURL(selectedApplicant.resume_url!)}
                    >
                      <View style={styles.resumeUrlIcon}>
                        <Ionicons name="link-outline" size={22} color="#8b5cf6" />
                      </View>
                      <View style={styles.resumeUrlInfo}>
                        <Text style={styles.resumeUrlLabel}>Google Drive Resume</Text>
                        <Text style={styles.resumeUrlText} numberOfLines={1}>
                          {selectedApplicant.resume_url}
                        </Text>
                      </View>
      ...
      {/* Table View Modal - All Applicants Details */}
      {/* Table View Modal - All Applicants Details */}
      <Modal
        visible={showTableView}
        transparent={true}
        animationType="slide"
        onRequestClose={() => setShowTableView(false)}
      >
        <View style={styles.modalOverlay}>
          <Pressable style={StyleSheet.absoluteFill} onPress={() => setShowTableView(false)} />
          <View style={[styles.applicantModal, { maxHeight: '80%', marginTop: 80 }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>All Applicants ({applicants.length})</Text>
              <View style={{ flexDirection: 'row', gap: 16, alignItems: 'center' }}>
                <TouchableOpacity
                  onPress={handleDownloadTable}
                  style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.cardAlt, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 }}
                >
                  <Ionicons name="download-outline" size={18} color="#6b46c1" />
                  <Text style={{ color: colors.primary, fontWeight: '600', fontSize: 13 }}>Download JSON</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setShowTableView(false)}>
                  <Ionicons name="close" size={24} color={colors.text} />
                </TouchableOpacity>
              </View>
            </View>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                <View style={{ minWidth: 800 }}>
                  ...
                  <View style={[styles.tableRow, styles.tableHeader]}>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 140 }]}>Name</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 40 }]}>Age</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 70 }]}>Gender</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 120 }]}>Phone</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 180 }]}>Address</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 160 }]}>Skills</Text>
                    <Text style={[styles.tableCell, styles.tableHeaderText, { width: 100 }]}>Resume</Text>
                  </View>
                  ...
                  {applicants.map((applicant, index) => (
                    ...
                    >
                      ...
                      <Text style={[styles.tableCell, { width: 40 }]}>
                        {applicant.id_card?.age ? applicant.id_card.age : calculateAge(applicant.id_card?.date_of_birth)}
                      </Text>
                      <Text style={[styles.tableCell, { width: 70 }]}>
                        {applicant.id_card?.gender || 'N/A'}
                      </Text>
                      <Text style={[styles.tableCell, { width: 120 }]} numberOfLines={1}>
                        {applicant.id_card?.phone_number || 'N/A'}
                      </Text>
                      <Text style={[styles.tableCell, { width: 180 }]} numberOfLines={3}>
                        {applicant.id_card?.address || 'N/A'}
                      </Text>
                      <Text style={[styles.tableCell, { width: 160 }]} numberOfLines={4}>
                        {applicant.id_card?.skills?.join(', ') || 'N/A'}
                      </Text>
                      <View style={[styles.tableCell, { width: 100 }]}>
                        {applicant.resume_url || applicant.resume?.file_url ? (
                          <TouchableOpacity onPress={() => Linking.openURL(applicant.resume_url || applicant.resume!.file_url)}>
                            <Ionicons name="link" size={24} color="#6b46c1" />
                          </TouchableOpacity>
                        ) : (
                          <Text style={styles.tableCellMuted}>No Resume</Text>
                        )}
                      </View>
                    </View>
                  ))}

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING(f"📊 Current database state:")
        )
        self.stdout.write(f"   - Messages: {message_count}")
        
        if message_count == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ Database is already clean - no messages to delete")
            )
            return
        ...
        if not options['force']:
            self.stdout.write(
                self.style.WARNING("\n⚠️  WARNING: This will delete ALL messages!")
            )
            self.stdout.write("⚠️  Conversations will be kept intact")
        ...
        try:
            ...
            if remaining_messages == 0:
                ...
                self.stdout.write(
                    self.style.WARNING("\n⚠️ Some messages may still exist - manual cleanup may be needed")
                )
